...
from .utils import show_localization_on_image
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        now  = time.strftime("%c")
        title = f'================ {now} ================\n'
        info  = f'{opt.dataset}, {opt.nz_T}, {opt.w_adv}, {opt.w_con}, {opt.w_lat}\n'
    ...
    def save_localization_images(self, model_student,grad):

        cnt=0
        labels = []
        predicted = []
        for i, data in enumerate(model_student.data.valid, 0):
            for j in range((data[0].shape[0])):
                img_add = model_student.data.valid.dataset.samples[cnt][0]
                img_lab = model_student.data.valid.dataset.samples[cnt][1]
                labels.append(img_lab)
                img_name = os.path.basename(img_add)
                
                #read corresponding input image separately
                input_img = cv2.imread(img_add)

                float_image = input_img.astype(np.float32)
                float_image /= 255.
                img= show_localization_on_image(float_image, grad[cnt], use_rgb=True)
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

                if self.opt.localization_method == 'gradients':
                    directory = os.path.join(self.tst_img_dir, 'vanilla_grad')
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    cv2.imwrite( os.path.join(directory ,img_name),img)

                elif self.opt.localization_method == 'smooth_grad':
                    directory = os.path.join(self.tst_img_dir, 'smooth_grad')
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    cv2.imwrite( os.path.join(directory , img_name),img)

                elif self.opt.localization_method == 'gbp':
                    directory = os.path.join(self.tst_img_dir, 'gbp')
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    cv2.imwrite( os.path.join(directory , img_name),img)
